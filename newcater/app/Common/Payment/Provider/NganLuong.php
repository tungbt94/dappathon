...
namespace Common\Payment\Provider;

use Common\Exception\Exception;

class NganLuong
{
    const RELEASE_PAYMENT_TYPE = 1;
    const HOLD_PAYMENT_TYPE = 2;
    ...
    function __construct($merchant_id, $merchant_password, $receiver_email, $productEnv = false)
    {
        $this->version = '3.1';
        $this->url_api = $productEnv
     ...
     */
    function VisaCheckout($order_code, $total_amount, $bank_code, $payment_type, $order_description, $tax_amount,
    ...
    function CheckoutCall($post_field)
    {
        ...
        if ($result != '' && $status == 200) {
            ...
            if ($nl_result->error_code != '00') {
                throw new Exception($this->GetErrorMessage($nl_result->error_code));
            }
        } else {
